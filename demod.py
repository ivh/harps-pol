...
def save(fname, X,Xe,I,Ie,N=None,Ne=None):
    ...
    else:    print('Something fishy with filenames or their ordering!')
...
def check_wl(wls, delta=0.1):
    ...
    for order in range(wlref.shape[0]):
        try:
            assert np.allclose([wl[order][0] for wl in wls],wlref[order][0],atol=delta)
...
def demod2(fnames):
    ...
    for i in range(Rb.shape[0]):
        Rb[i] = interp1d(w1b[i],Rb[i].filled(np.nan),fill_value='extrapolate', bounds_error=False)(w1a[i])
    R = Ra*Rb
    X = np.sqrt(R) - 1
    X /= np.sqrt(R) + 1
...
def demod4(fnames):
    ...
    Ie = 0.125 * np.sqrt(e1a**2 + e1b**2 + e2a**2 + e2b**2 + e3a**2 + e3b**2 + e4a**2 + e4b**2)
    ...
    # Error propagation
    # First calculate Ra and Rb errors using quotient rule
    Rae = Ra * np.sqrt((e1b/s1b)**2 + (e2b/s2b)**2 + (e4a/s4a)**2 + (e3a/s3a)**2)
    Rbe = Rb * np.sqrt((e2a/s2a)**2 + (e1a/s1a)**2 + (e3b/s3b)**2 + (e4b/s4b)**2)
    Re = R * np.sqrt((Rae/Ra)**2 + (Rbe/Rb)**2)
